...
- **Architecture Baseline**: Steve Smith / Ardalis Clean Architecture template style
- **Endpoint Style**: FastEndpoints-style vertical endpoint classes with thin HTTP adapters
...
### 1.3 Backend Architecture - Clean Architecture with FastEndpoints and CQRS

The backend services use the Steve Smith / Ardalis Clean Architecture template style. HTTP endpoints are implemented as FastEndpoints-style vertical endpoint classes that delegate to application use cases. CQRS/MediatR is used where it clarifies document, artifact, and generation workflows.
...
#### FastEndpoints-Style Endpoint Benefits
- **Thin HTTP Layer**: Endpoints translate HTTP input/output and delegate business work to use cases
- **Feature Locality**: Request, response, validator, and endpoint logic stay close together
- **OpenAPI Friendly**: Endpoint metadata drives Swagger/OpenAPI documentation
- **Clean Architecture Fit**: Endpoint classes remain in the API layer and do not own domain behavior

#### MediatR Pattern Benefits
- **Decoupling**: Endpoints don't depend on specific business logic implementations
